...
import { getBloggerCompanies } from "./functions";
import {
  bloggerCompaniesSelectOptions,
  bloggerCompaniesStatuses
} from "../../constants/bloggerCompaniesStatuses";
...
const Blogger = () => {
  ...
  return <div>
    <Typography variant="h3">
      {formatMessage({ id: "pages.myCompanies" })}
    </Typography>
    <Flex className={classes.filters} between container column={!md} marginBottom={!md} spacing={20}>
      <Flex xs="100%" lg="300px">
        <BaseSelect
          ...
          options={bloggerCompaniesSelectOptions}
        />
      </Flex>
    ...
    <div>
      {console.log(getBloggerCompanies(myCompanies, companiesFilter, search))}
      <LoaderWrap loading={loadingCompanies || loadingMe}>
        {
          getBloggerCompanies(myCompanies, companiesFilter, search).length
            ? getBloggerCompanies(myCompanies, companiesFilter, search).map(({ status, companies }) => {
              return (
                <React.Fragment key={status}>
                  <div className={classes.status}>
                    {formatMessage({ id: bloggerCompaniesStatuses[status] || status })}
                  </div>
                          ...
                          <Flex center="xs" nowrap container spacing={15} middle="xs" >
                            ...
                            <Flex>
                              <Tooltip title={formatMessage({ id: "tooltip.edit" })}>
                                <span onClick={() => push(`companies/edit/${company.id}`)}>
                                  <Edit />
                                </span>
                              </Tooltip>
                            </Flex>
